...
vec4 dither(vec4 color, vec2 fragCoord) {
  int xbias = int(mod(fragCoord.x, 4.0));
  int ybias = int(mod(fragCoord.y, 4.0));

  float bias = ditherMatrix[xbias + ybias * 4];

  return vec4(
    round(color.r * 32.0 + bias) / 32.0,
    round(color.g * 32.0 + bias) / 32.0,
    round(color.b * 32.0 + bias) / 32.0,
    color.a
  );
}
